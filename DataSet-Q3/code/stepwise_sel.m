function y_pred = stepwise_sel(data1,data2)
    [n,m] = size(data1.CASE1201); 
    [n2,m] = size(data2.CASE1201); 
    % %Version of using intercept and coding states
%     for i=1:n
%         data1.CASE1201{i,1}= i;
%         if i<=n2
%             data2.CASE1201{i,1}= i;
%         end
%     end
%     x = str2double(data1.CASE1201{:,[1,3:8]});
%     x = [x,ones(n,1)];
%     x_test = str2double(data2.CASE1201{:,[1,3:8]});

    % Version of using intercept but no states
    x = data1.CASE1201{:,3:8};
    x = [x,ones(n,1)];
    m = m-1;
    x_test =  data2.CASE1201{:,3:8};
    y = data1.CASE1201{:,2};
